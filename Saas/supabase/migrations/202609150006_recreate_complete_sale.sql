-- Run this in Supabase SQL Editor if complete_sale is missing from the schema cache.
...
begin
  ...
  for item in select value from jsonb_array_elements(sale_items) loop
    ...
    item_price := coalesce((select pp.price from public.product_prices pp where pp.product_id = product_row.id and pp.tenant_id = target_tenant_id and (pp.effective_to is null or pp.effective_to > now()) order by pp.effective_from desc limit 1), 0);
    ...
    if product_row.track_inventory then
      ...
      where tenant_id = target_tenant_id and location_id = current_location_id and product_id = product_row.id and quantity >= item_quantity;
