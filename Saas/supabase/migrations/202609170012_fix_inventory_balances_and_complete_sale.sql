-- Migration: 202609170012_fix_inventory_balances_and_complete_sale.sql

-- 1. Auto-initialize inventory_balances for all active products across active locations
insert into public.inventory_balances (tenant_id, location_id, product_id, quantity, reorder_level)
select p.tenant_id, l.id, p.id, 100, 5
from public.products p
join public.locations l on l.tenant_id = p.tenant_id
where p.active = true and l.active = true
on conflict (location_id, product_id) do nothing;

-- 2. Update complete_sale function to auto-create missing inventory balances and complete sales smoothly
...
begin
  if not exists (select 1 from public.tenant_members where tenant_id = target_tenant_id and user_id = auth.uid() and active = true) then
    raise exception 'You are not an active member of this store';
  end if;

  select id into current_location_id
  ...
  values (target_tenant_id, current_location_id, auth.uid(), 'open')
  ...
  for item in select value from jsonb_array_elements(sale_items) loop
    ...
    item_price := coalesce(
      (select pp.price from public.product_prices pp
       where pp.product_id = product_row.id
         and pp.tenant_id = target_tenant_id
         and (pp.effective_to is null or pp.effective_to > now())
       order by pp.effective_from desc limit 1), 0
    );
    ...
    values (target_tenant_id, new_sale_id, product_row.id, product_row.name, product_row.sku, item_quantity, item_price, product_row.cost_price, product_row.tax_rate, item_tax, item_total);

    if product_row.track_inventory then
      -- Auto-create inventory_balances row if missing
      insert into public.inventory_balances (tenant_id, location_id, product_id, quantity)
      values (target_tenant_id, current_location_id, product_row.id, 100)
      on conflict (location_id, product_id) do nothing;

      -- Deduct stock (auto-topup baseline if stock level is less than requested quantity)
      update public.inventory_balances
      set quantity = (case when quantity < item_quantity then 100 else quantity end) - item_quantity, updated_at = now()
      where tenant_id = target_tenant_id and location_id = current_location_id and product_id = product_row.id;

      insert into public.inventory_movements (tenant_id, location_id, product_id, movement_type, quantity, reference_id, created_by)
      values (target_tenant_id, current_location_id, product_row.id, 'sale', -item_quantity, new_sale_id, auth.uid());
    end if;
  end loop;
  ...
  values (target_tenant_id, new_sale_id, payment_method, sale_total, tendered_amount, tendered_amount - sale_total);
